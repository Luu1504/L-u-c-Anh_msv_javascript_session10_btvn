...
let getFilteredTotalGoals = function (minGoals, teamPlayers) {
    let tongBanThang = 0;

    for (let i = 0; i < teamPlayers.length; i++) {
        let cauThu = teamPlayers[i];

        if (cauThu.goals >= minGoals) {
            tongBanThang = tongBanThang + cauThu.goals;
        }
    }
...
let ketQua1 = getFilteredTotalGoals(5, players);
console.log(`Tổng các bàn thắng của các cầu thủ đạt đủ chỉ tiêu là : ${ketQua1}`);

let ketQua2 = getFilteredTotalGoals(50, players);
console.log(`Tổng các bàn thắng của các cầu thủ đạt đủ chỉ tiêu là : ${ketQua2}`);